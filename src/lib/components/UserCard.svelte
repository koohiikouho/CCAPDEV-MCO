...
<Card href="/src/routes/profiles/viewProfile.html?profile=1" class="p-4 sm:p-5 md:p-7 min-w-100 bg-primary-50/20 rounded-md hover:opacity-80 hover:scale-110 hover:border-[0.5px] hover:border-primary-600 transition-transform shadow-none duration-100 cursor-pointer hover:shadow-lg hover:shadow-secondary-200/50
  ...
  <div class="flex flex-col items-center pb-4">
    <Avatar size="lg" src={avatarSrc} />
      <h5 class="mb-2 text-2xl tracking-tight text-surface-500 dark:text-white font-medium">{name}</h5>
      <span class="mb-3 text-xs tracking-tight text-surface-400 dark:text-gray-400 font-normal">{email}</span>
      <span class="mb-3 text-xs tracking-tight text-surface-400 dark:text-gray-400 font-normal">{role}</span>
