<script lang="ts">
  ...
  let {
    ...
    role,
    usercode,
  }: { name: string; email: string; avatar: string; role: string; usercode } =
    $props();
...
>
  ...
  <div class="flex flex-col items-center pb-4 pt-2">
    ...
    <span
      class="mb-3 text-xs tracking-tight text-surface-400 dark:text-gray-400 font-normal"
      >{email}</span
    >
    <span
      class="text-xs tracking-tight text-surface-400 dark:text-gray-400 font-normal"
      >{role}</span
    >
  </div>
</Card>
